module NavigationHelpers
  ...
  def path_to(page_name)
    ...
    when /the events page for plate with barcode "(\d+)"/
      asset = Plate.find_from_machine_barcode($1)
      history_asset_path(asset)

    when /the events page for tube with barcode "(\d+)"/
      asset = Tube.find_from_machine_barcode($1)
      history_asset_path(asset)
