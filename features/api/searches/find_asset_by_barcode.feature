...
Feature: Searching for assets by barcode
  ...
  Scenario Outline: Looking up a single asset by barcode
    Given the UUID for the search "Find <search> by barcode" is "00000000-1111-2222-3333-444444444444"

    Given a <asset_type> called "Testing the API 1" with ID 1
      And the <asset_type> "Testing the API 1" has a barcode of "<barcode>"
      And the UUID for the <model_name> "Testing the API 1" is "11111111-2222-3333-4444-000000000001"

    Given a <asset_type> called "Testing the API 2" with ID 2
      And the <asset_type> "Testing the API 2" has a barcode of "<barcode_2>"
      And the UUID for the <model_name> "Testing the API 2" is "11111111-2222-3333-4444-000000000002"
    ...
    Then the HTTP response should be "301 Moved Permanently"
     And the JSON should match the following for the specified fields:
      ...
      """
    Examples:
      | asset_type             | model_name  | barcode       | barcode_2     | search |
      | control plate          | plate       | 1220000123724 | 1220000999701 | plates |
      | dilution plate         | plate       | 1220000123724 | 1220000999701 | plates |
      | gel dilution plate     | plate       | 1930000123708 | 1930000999686 | plates |
      | pico assay a plate     | plate       | 4330000123802 | 4330000999780 | plates |
      | pico assay b plate     | plate       | 4340000123849 | 4340000999826 | plates |
      | pico assay plate       | plate       | 4330000123802 | 4330000999780 | plates |
      | pico dilution plate    | plate       | 4360000123694 | 4360000999671 | plates |
      | plate                  | plate       | 1220000123724 | 1220000999701 | plates |
      | working dilution plate | plate       | 6250000123818 | 6250000999796 | plates |
      | sample tube            | sample_tube | 3980012344750 | 3980012345764 | tubes  |



  @single @sample_tube @error
  Scenario: Looking up a non-existant barcode
    Given the UUID for the search "Find plates by barcode" is "00000000-1111-2222-3333-444444444444"
  ...
  Scenario: Looking up multiple assets by barcode
    Given the UUID for the search "Find tubes by barcode" is "00000000-1111-2222-3333-444444444444"
