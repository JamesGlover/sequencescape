  ...
  def self.included(base)
    base.class_eval do
      scope :requiring_fluidigm_data, -> {
        ...
        select('labware.*, plate_metadata.fluidigm_barcode AS fluidigm_barcode')
          .joins([
            'INNER JOIN plate_metadata ON plate_metadata.plate_id = labware.id AND plate_metadata.fluidigm_barcode IS NOT NULL', # The fluidigm metadata
            ...
            'LEFT OUTER JOIN events ON eventful_id = labware.id AND eventful_type = "Labware" AND family = "update_fluidigm_plate" AND content = "FLUIDIGM_DATA" '
          ])
