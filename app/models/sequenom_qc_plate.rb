class SequenomQcPlate < Plate
  ...
  attr_writer :input_plate_names

  before_validation :assign_name, :on => :create
  validates_presence_of :name
  validate :source_plates_genders_valid?, :if => :do_gender_checks?, :on=> :create
  validate :at_least_one_source_plate?, :on => :create
  validate :input_plates_exist?, :on => :create
  validate :user_barcode_exist?, :on => :create

  after_create :connect_input_plates
  ...
  def generate_name
    ...
    "#{plate_prefix}#{plate_number(input_plate_names)}#{plate_date}" unless input_plate_names.nil?
  end
  ...
  def source_plates_genders_valid?
    input_plate_names.each do |source_plate_number,source_plate_barcode|
      next if source_plate_barcode.blank?
